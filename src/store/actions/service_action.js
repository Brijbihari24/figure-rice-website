...
import {
    ...
    GET_SIMILAR_SERVICE,
    GET_SIMILAR_SERVICE_STATED,
    GET_SIMILAR_SERVICE_ENDED,
...
export const getServiceBySlug = (id) => async (dispatch) => {
    try {
        dispatch({
            type: GET_SERVICE_STATED,
        });
        const { data } = await api.get(`/services/slug/${id}`);
...
export const getSimilarService = (id) => async (dispatch) => {
    try {
        dispatch({
            type: GET_SIMILAR_SERVICE_STATED,
        });
        const { data } = await api.get(`/services/similar/${id}`);

        dispatch({
            type: GET_SIMILAR_SERVICE,
            payload: data,
        });
        dispatch({
            type: GET_SIMILAR_SERVICE_ENDED,
        });
    } catch (error) {
        dispatch({
            type: GET_SIMILAR_SERVICE_ENDED,
    ...
    ({ term, value }) =>
        async (dispatch) => {
            try {
                ...
                const { data } = await api.get(`/services/all?term=${term}&value=${value}`);

...
import {
  ...
  GET_ALL_BLOGS_ENDED,
  GET_SIMILAR_BLOG,
  GET_SIMILAR_BLOG_STATED,
  GET_SIMILAR_BLOG_ENDED,
...
export const getBlogBySlug = (id) => async (dispatch) => {
  try {
    dispatch({
      type: GET_BLOG_STATED,
    });
    const { data } = await api.get(`/blogs/slug/${id}`);
...
export const getSimilarBlog = (id) => async (dispatch) => {
  try {
    dispatch({
      type: GET_SIMILAR_BLOG_STATED,
    });
    const { data } = await api.get(`/blogs/similar/${id}`);

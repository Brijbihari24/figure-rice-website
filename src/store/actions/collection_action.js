...
export const getCollectionBySlug = (id) => async (dispatch) => {
    try {
        dispatch({
            type: GET_COLLECTION_STATED,
        });
        const { data } = await api.get(`/collections/slug/${id}`);
    ...
    ({ term, value }) =>
        async (dispatch) => {
            try {
                ...
                const { data } = await api.get(`/collections/all?term=${term}&value=${value}`);
